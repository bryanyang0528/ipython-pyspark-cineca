...
RUN service ssh start && $HADOOP_PREFIX/sbin/stop-all.sh && $HADOOP_PREFIX/etc/hadoop/hadoop-env.sh && $HADOOP_PREFIX/sbin/start-dfs.sh && $HADOOP_PREFIX/bin/hdfs dfsadmin -safemode leave && $HADOOP_PREFIX/bin/hdfs dfs -put /usr/local/spark-1.4.1-bin-hadoop2.4/lib /spark
